...
def plot_crack(cracktip_1, cracktip_2, length_element):
    plt.scatter(cracktip_1, cracktip_2, c="b", s=100)
    plt.plot(cracktip_1, cracktip_2, linewidth=3, c="r", label = "Crack")
    # lc_R = [cracktip_1[1] + length_element , cracktip_2[1]]
    # print(lc_R)
    # lc_L = [cracktip_1[1], cracktip_2[1]]
    # segment = np.array([lc_R, lc_L])
    # print(lc_L)
    # plt.scatter(segment[0:,0], segment[0:,1])
    # plt.plot(segment[0:,0], segment[0:,1], linewidth=3, c="darkblue", label = "Damage perimeter")
    # ax = plt.gca()
    # circle1 = plt.Circle(lc_R, length_element,linewidth=3, color='darkblue', fill=False)
...
def heavi_dofs(echalon):
    x = np.asarray(echalon)
    plt.scatter(x[:,0], x[:, 1], c='grey', marker='o', s = 100, label = "Heavy_enriched")
    # plt.legend(loc="upper right")
