...
def force(DISPS):
    ...
    #forming it as an array
    Z = np.asarray(DISPS)
    plt.scatter(Z[:,0], Z[:,1], c='orange', s=200, label = "Displacements")
    plt.legend(loc="upper right")
...
def contour_plot(K, S1, A, No):
    ...
    ax1.set_title('Distribution of $\sigma_x$, N/mm2')
    ...
    # plt.show()
    # fig.savefig(f"Stress_plot_{No}.png")
