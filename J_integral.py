...
def Interaction_integral(Nodes, displacements, stress, set4Gauss, c_2, GaussPoint_1to4,
                             l_x, l_y, D_plane_stress, alpha, CL, A, DISP, D, scale):
    ...
    K_1 = (I1_2 * D)/2
    print("K1 is", K_1/np.sqrt(1000))

    stress = (DISP*1e-3*D)/2
    K_TH = (1.125 * (stress * m.sqrt(np.pi*CL*1e-3)))

    print("theory in MPa root(m)", K_TH)
    ...
    # Q = force
    # Ist = (1/(m.sqrt(np.pi*CL))) * (force * F1)
    # IInd = (1/(m.sqrt(np.pi*CL))) * (Q * 2* F2/np.pi)
    # KI_th = Ist - IInd

    # IIIrd = (1/(m.sqrt(np.pi*CL))) * (Q * F3)
    # IVth = (1/(m.sqrt(np.pi*CL))) * (force * 2* F2/np.pi)
    # KII_th = IIIrd - IVth
    # print("theoretical SIF", KI_th)
    # print("theoretical SIF", KII_th)
