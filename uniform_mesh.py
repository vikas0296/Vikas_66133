...
if __name__ == "__main__":
    ...
    ===========================================================================
    '''
    cracktip_1 = np.array([1, 2.3])
    cracktip_2 = np.array([2.3, 2.3])
...
#-------------------------------single element and it is Tip enriched --------------------------------------------------
    ...
    Tip_displacements1 = Displacement_approx.displacement_approximation(list(T_node), c_1, c_2, list(T_elem), r, theta,
                                              alpha, Tside, Hside, C_Dofs, displacement_vector1)

    Tip_stress1 = Stress_Strains.strain_stress_tensors(Tip_displacements1, list(U_L), GaussPoint_1to4, D_plane_stress)
...
#-----------------------------If the crack length is more than element length-------------------------------------
    ...
    P_displacements1 = Displacement_approx.displacement_approximation(list(P_node), c_1, c_2, list(P_elm), r, theta,
                                                              alpha, Tside, Hside, C_Dofs, displacement_vector1)

    for i,j in zip(list(S_U), list(S_L)):
        Q.append(i)
        Q.append(j)
        Partial_stress_U1 = Stress_Strains.strain_stress_tensors(P_displacements1, i, GaussPoint_1to4, D_plane_stress)
        Partial_stress_L1= Stress_Strains.strain_stress_tensors(P_displacements1, j, GaussPoint_1to4, D_plane_stress)
...
#-------------------------------------------------------------------------------------------------------------------------
    ...
    H_displacements1 = Displacement_approx.displacement_approximation(list(H_node), c_1, c_2, list(H_elm), r, theta,
                                                          alpha, Tside, Hside, C_Dofs, displacement_vector1)
    ...
    for i,j in zip(list(H_U), list(H_L)):
        ...
        P.append(j)
        # print(i)
        Heavy_stress_U1 = Stress_Strains.strain_stress_tensors(H_displacements1, i, GaussPoint_1to4, D_plane_stress)
        Heavy_stress_L1 = Stress_Strains.strain_stress_tensors(H_displacements1, j, GaussPoint_1to4, D_plane_stress)
...
# #--------------------------------stress_strain for the unenriched elements-------------------------------------------------
    ...
    A_stress = Stress_Strains.strain_stress_tensors(A_displacement1,influenced_elements, GaussPoint_1to4, D_plane_stress)
...
#     # #stress_strain for partially cut or fully cut element if crack length < element length
    ...
    # Tip_displacements2 = Displacement_approx.displacement_approximation(list(T_node), c_1, c_2, list(T_elem), r, theta,
    #                                           alpha, Tside, Hside, C_Dofs, displacement_vector2)

    # Tip_stress2 = Stress_Strains.strain_stress_tensors(Tip_displacements2, list(U_L), GaussPoint_1to4, D_plane_stress)


# #-----------------------------If the crack length is more than element length-------------------------------------
#     # #stress_strain for partially cut or fully cut element if crack length > element length

    # P_displacements2 = Displacement_approx.displacement_approximation(list(P_node), c_1, c_2, list(P_elm), r, theta,
    #                                                           alpha, Tside, Hside, C_Dofs, displacement_vector2)

    # for i,j in zip(list(S_U), list(S_L)):
    #     Partial_stress_U2 = Stress_Strains.strain_stress_tensors(P_displacements2, i, GaussPoint_1to4, D_plane_stress)
    #     Partial_stress_L2 = Stress_Strains.strain_stress_tensors(P_displacements2, j, GaussPoint_1to4, D_plane_stress)

# #-------------------------------------------------------------------------------------------------------------------------
#     # stress_strain for heaviside enriched elements, if crack length > element length

    # H_displacements2 = Displacement_approx.displacement_approximation(list(H_node), c_1, c_2, list(H_elm), r, theta,
    #                                                       alpha, Tside, Hside, C_Dofs, displacement_vector2)

    # for i,j in zip(list(H_U), list(H_L)):
    #     Heavy_stress_U = Stress_Strains.strain_stress_tensors(H_displacements2, i, GaussPoint_1to4, D_plane_stress)
    #     Heavy_stress_L = Stress_Strains.strain_stress_tensors(H_displacements2, j, GaussPoint_1to4, D_plane_stress)
