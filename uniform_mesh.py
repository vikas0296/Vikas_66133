...
if __name__ == "__main__":
    ...
    B = 10
    x = 15                                #3x3 will be elements, 4x4 will be nodes
    ...
    plots.plot_mesh(NL, A, B, x, length_element)
    ...
    ===========================================================================
    '''
    cracktip_1 = np.array([1, (B+1)/2])
    cracktip_2 = np.array([(A+1)/2, (A+1)/2])
    ...
    force1 = 2448
    displacement_vector1, BCs = new_crack.Boundary_conds(NL, A, B, length_element, K_global, force1)
...
#-------------------------------single element and it is Tip enriched --------------------------------------------------
    ...
    Tip_displacements1 = Displacement_approx.displacement_approximation(T_n, c_1, c_2, T_e,Tside, Hside,
                                                                        C_Dofs, displacement_vector1, alpha)

    Tip_stress1, strainsTP = Stress_Strains.strain_stress_enr(Tip_displacements1, upperlower, GaussPoint_1to4, D_plane_stress)
...
#-----------------------------If the crack length is more than element length-------------------------------------
    ...
    P_displacements1 = Displacement_approx.displacement_approximation(P_n, c_1, c_2, P_e,
                                                                      Tside, Hside, C_Dofs, displacement_vector1, alpha)

    for i,j in zip(list(S_U), list(S_L)):
        Q.append(i)
        Q.append(j)
        Partial_stress_U1, strainsPU = Stress_Strains.strain_stress_enr(P_displacements1, i, GaussPoint_1to4, D_plane_stress)
        Partial_stress_L1, strainsPL = Stress_Strains.strain_stress_enr(P_displacements1, j, GaussPoint_1to4, D_plane_stress)
...
#-------------------------------------------------------------------------------------------------------------------------
    ...
    H_displacements1 = Displacement_approx.displacement_approximation(H_n, c_1, c_2, H_e,
                                                         Tside, Hside, C_Dofs, displacement_vector1, alpha)
    ...
    for i,j in zip(list(H_U), list(H_L)):
        P.append(list(H_U))
        P.append(list(H_U))
        Heavy_stress_U1, strainsHU = Stress_Strains.strain_stress_enr(H_displacements1, i, GaussPoint_1to4, D_plane_stress)
        Heavy_stress_L1, strainsHL = Stress_Strains.strain_stress_enr(H_displacements1, j, GaussPoint_1to4, D_plane_stress)
        STRESS1.append(Heavy_stress_U1)
        STRESS1.append(Heavy_stress_L1)

# #--------------------------------stress_strain for the unenriched elements-------------------------------------------------

    A_displacement1 = Displacement_approx.displacement_approximation(influenced_elements, c_1, c_2, elems,
                                                            Tside, Hside, C_Dofs, displacement_vector1, alpha)

    A_stress1, strains1 = Stress_Strains.strain_stress_tensors(A_displacement1,influenced_elements, GaussPoint_1to4, D_plane_stress)
...
#==========================================================================================================================================
    r, P1, P2, P3, P4, P5 = Assignment.intersect(c_2, length_element)
    ygod = []
    egod = []
    for i,j in zip(all_ns, elements):
        E1 = i[0]
        ...
        E4 = i[3]
        for k in i:
            Z = J_integral.inside_rect(P5, P3, k)
            if Z == True:
                Quad = [E1, E2, E3, E4, E1]
                ygod.append(i)
                egod.append(j)
                plots.extra_dofs(Quad)



    b = []
    for i in Partial_node:
    ...
    for k in Heavy_node:
        if type(k).__name__ != "NoneType":
                b.append(k)


    # NODES = Assignment.waste(ygod, egod, b)
    Verschiebung = []
    Belastung = []
    Spannung = []
    for i,j in zip(ygod, egod):
        if i in b:
            DISPLACEMENTS_2 = Displacement_approx.displacement_approximation([i], c_1, c_2, [j], Tside, Hside,
                                                                             C_Dofs, displacement_vector1, alpha)

            STRESS_2, Spannung_2 = Stress_Strains.strain_stress_enr(DISPLACEMENTS_2, [i], GaussPoint_1to4, D_plane_stress)
            Verschiebung.append(DISPLACEMENTS_2)
            Belastung.append(STRESS_2)
            Spannung.append(Spannung_2)

        else:
            DISPLACEMENTS_NO = Displacement_approx.displacement_approximation([i], c_1, c_2, [j],Tside, Hside,
                                                                              C_Dofs, displacement_vector1, alpha)

            STRESS_NO, Spannung_NO = Stress_Strains.strain_stress_tensors(DISPLACEMENTS_NO, [i], GaussPoint_1to4, D_plane_stress)
            Verschiebung.append(DISPLACEMENTS_NO)
            Belastung.append(STRESS_NO)
            Spannung.append(Spannung_NO)


    G, GPs = Assignment.G_points(ygod)
    J_integral.Interaction_integral(ygod, Verschiebung, Belastung, Spannung, GPs, P2, P3, P4, P5, c_2, GaussPoint_1to4,
                                    D_plane_stress, alpha, crack_length, A, force1)
    ...
    # Z = np.linspace(min(zta), max(zta), 10000).reshape(100,100)
