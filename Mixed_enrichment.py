...
def mixed_enrichment(a, Nodes_U, Nodes_L, r, theta, alpha, GaussPoint_1to4, D_plane_stress):
    ...
    for i in Nodes_U:
        for points in GaussPoint_1to4:
            ...
            B_all_U = np.concatenate((B_std_U, heavy_1, tip_2, tip_3, heavy_4), axis=1)

            Bt_D_U = np.matmul(B_all_U.T, D_plane_stress)
            Bt_D_B_U = (np.matmul(Bt_D_U, B_all_U)) * linalg.det(jacobi)
            K_mixed_U = np.round(Bt_D_B_U, 3)
            Q = Bt_D_B_U.shape
    ...
    for j in Nodes_L:
        for points in GaussPoint_1to4:
            ...
            B_all_L = np.concatenate((B_std_L, heavy_1L, tip_2L, tip_3L, heavy_4L), axis=1)

            Bt_D_L = np.matmul(B_all_L.T, D_plane_stress)
            Bt_D_B_L = (np.matmul(Bt_D_L, B_all_L)) * linalg.det(jacobi)
            K_mixed_L = np.round(Bt_D_B_L, 3)
            R = Bt_D_B_L.shape
