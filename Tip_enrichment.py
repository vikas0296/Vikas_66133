...
def tip_enrichment(coordinates, r, theta, alpha, GaussPoint_1to4, D_plane_stress):
    '''
    ...
    for points in GaussPoint_1to4:
        ...
        B_tip2 = enrichment_functions.tip_enrichment_func_N2(F1, F2, F3, F4, dN, dF, N)

        #B-matrix for additional DOFs (8 for node 3)
        B_tip3 = enrichment_functions.tip_enrichment_func_N3(F1, F2, F3, F4, dN, dF, N)

        # B-matrix for additional DOFs (8 for node 4)
        B_tip4 = enrichment_functions.tip_enrichment_func_N4(F1, F2, F3, F4, dN, dF, N)
